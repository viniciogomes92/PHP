...
function getCardapios($pdo, $itensPorPagina, $offset) {
    ...
    $sqlCount = "SELECT COUNT(*) as total FROM cardapio_jantar";
            ...
            <div class="card-body">
                ...
                <form method="POST" action="acoes_cardapios_jantar.php">
                    <div class="row g-3">
                        ...
                        <!-- Botão de Enviar -->
                        <div class="col-12">
                            <button type="submit" name="create_cardapioJantar" class="btn btn-primary">Cadastrar Cardápio</button>
                        </div>
                    </div>
                </form>
            </div>
                            ...
                            <tbody>
                                <?php foreach ($cardapios as $cardapio): ?>
                                    <tr class="cardapio-item">
                                        ...
                                        <td>
                                            <a href="cardapios_jantar-edit.php?id=<?= $cardapio['id_cardapio'] ?>&data_cardapio=<?= $cardapio['data_cardapio'] ?>" class="btn btn-sm btn-success mb-1"><span class="bi-pencil-fill"></span>Editar</a>
                                            
                                            <form action="./acoes_cardapios_jantar.php" method="POST" class="d-inline">
                                                <button onclick="return confirmaDeleteCardapioJantar(<?=$cardapio['id_cardapio']?>, '<?= $cardapio['data_cardapio'] ?>')" type="button" name="delete_cardapiojantar" value="<?=$cardapio['id_cardapio']?>" class="btn btn-danger btn-sm">
